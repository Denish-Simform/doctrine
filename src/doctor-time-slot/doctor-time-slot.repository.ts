...
export class DoctorTimeSlotRepository {
  ...
  async create(
    doctorTimeSlots: Partial<DoctorTimeSlot>[],
  ...
  async createBulk(doctorTimeSlots: any): Promise<DoctorTimeSlot[]> {
    ...
    for (const [scheduleId, slots] of Object.entries(doctorTimeSlots)) {
      const weeklySchedule = await this.doctorWeeklyScheduleRepository.findOne(
        scheduleId as UUID,
      );
  ...
  update(
    id: UUID,
    updateData: Partial<DoctorTimeSlot>,
  ): Promise<DoctorTimeSlot> {
  ...
  findAllByDoctor(doctorId: UUID): Promise<DoctorTimeSlot[]> {
    return this.doctorTimeSlotRepo
  ...
  findAllByDoctorAndDay(
    ...
    return this.doctorTimeSlotRepo
      .createQueryBuilder('time_slot')
      .select(
        'time_slot.date, time_slot.start_time, time_slot.end_time, time_slot.id, time_slot.status, time_slot.schedule_id',
      )
      .leftJoin(
        'doctor_weekly_schedule',
        'schedule',
