...
export class DoctorWeeklyScheduleRepository extends BaseRepository {
  ...
  constructor(
    ...
    super();
    this.doctorWeeklyScheduleRepo =
      dataSource.getRepository(DoctorWeeklySchedule);
  }

  async create(doctorWeeklySchedule: any): Promise<DoctorWeeklySchedule[]> {
    const newDoctorWeeklySchedules = doctorWeeklySchedule.map(
      (schedule: Partial<DoctorWeeklySchedule>) => {
  ...
  findOneBy(
    condition: Partial<DoctorWeeklySchedule>,
  ): Promise<DoctorWeeklySchedule | null> {
  ...
  update(id: UUID, updateData: any): Promise<DoctorWeeklySchedule[]> {
    ...
    return this.findAllByDoctorId(id).then((doctorWeeklySchedules) => {
      ...
      const updatedDoctorWeeklySchedule = doctorWeeklySchedules.map(
        (doctorWeeklySchedule, index) => {
          if (
            doctorWeeklySchedule.day_of_week !==
            updateData.weekly_schedule[index].day_of_week
          ) {
            return doctorWeeklySchedule;
          }
          return this.updateEntityRecursively(
            doctorWeeklySchedule,
            updateData.weekly_schedule[index],
          );
        },
      );
      console.log(
        'Updated Doctor Weekly Schedule:',
        updatedDoctorWeeklySchedule,
      );
